...
export default function push (state, {payload}) {
  ...
  const { location = {}, params = {}, routes, limit, exclude } = payload
  ...
  const breadcrumb = mapBreadcrumb(current, location, { ...params }, limit, exclude)
  ...
  if (breadcrumbs.length > limit && location.action === 'PUSH') {
