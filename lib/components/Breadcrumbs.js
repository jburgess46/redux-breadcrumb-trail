...
import { array, func, node, oneOfType, shape, number } from 'prop-types'
import { connect } from 'react-redux'

import BreadcrumbItem from './BreadcrumbItem'
import { push, splice } from '../actionCreators'
...
export class Breadcrumbs extends Component {
  ...
  refreshBreadcrumb = props => {
    const { dispatch, routes, params, location, limit, exclude } = props
    if (!isMatch(this.state.previousLocation, location)) {
      dispatch(push({ location, params, routes, limit, exclude }))
